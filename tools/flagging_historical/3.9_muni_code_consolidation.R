...
pr_area_comparisons_mer$MER_latitude <- 
  str_sub(pr_area_comparisons_mer$MER_lat, end = -2)
pr_area_comparisons_mer$MER_longitude <- 
  str_sub(pr_area_comparisons_mer$MER_long, end = -2)
pr_area_comparisons_mer$MER_longitude <- 
  as.numeric(as.character(pr_area_comparisons_mer$MER_longitude))
...
pr_area_comparisons_mer$MER_latitude_d <- 
  as.numeric(as.character(pr_area_comparisons_mer$MER_latitude_d))
...
spatialjoin_skeleton <- 
  pr_mrip_mer_area_comparison_spatialjoin[, c(1, 2, 6, 19, 20, 37, 44, 45)]

# remove rows from MER that have na's as those locations are not used
pr_mer_clean <- na.omit(pr_mer_skeleton)

# # try full join
# # only one location with the same site name in both tables, 
      # figure out another way to cross reference

# merge to add mer locations not in the join
combo_merge_mer <- merge(spatialjoin_skeleton,
                         pr_mer_clean, 
                         by = "MER_Name", 
                         all = TRUE)
...
combo_merge_mermrip <- merge(combo_merge_mer_clean, pr_mrip_skeleton,
  by = "MRIP_Site_Name",
  all = TRUE
)

# clean columns of mrip merge
combo_merge_mermrip_clean <-
  combo_merge_mermrip[, c(1, 2, 3, 4, 7, 8, 9, 10, 11, 12)]
write.csv(combo_merge_mermrip_clean,
  file = "tools/output/combo_merge_firstdraft.csv",
  row.names = FALSE
)

# read in cleaned first draft (combined mer/mrip locations fixed)
combo_merge_firstdraft_edited <- 
  read_csv("tools/output/combo_merge_firstdraft_edited.csv")

# create a new column for official site name
combo_merge <- combo_merge_firstdraft_edited |>
  mutate(
    SITE_NAME = case_when(
      !is.na(MER_Name) ~ MER_Name,
      TRUE ~ MRIP_Site_Name
    ),
    MUNICIPIO = case_when(
      !is.na(MER_Municipio) ~ MER_Municipio,
      TRUE ~ MRIP_County
    ),
    GEN_LAT = case_when(
      !is.na(MER_latitude_d) ~ MER_latitude_d,
      TRUE ~ MRIP_Latitude.y
    ),
    GEN_LONG = case_when(
      !is.na(MER_longitued_d) ~ MER_longitued_d,
      TRUE ~ MRIP_Longitude.y
    )
  )
mer_mrip_merge_ID <- rowid_to_column(combo_merge, "SITE_ID") |>
