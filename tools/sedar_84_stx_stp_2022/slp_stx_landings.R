...
by_year_gear <- data.qcd |>
  ...
  mutate(
    PERCENT_LBS = round(TOTAL_LBS / YEAR_LBS, 4),
...
by_year_tip_percentage <- by_year_tip |>
  ...
  mutate(
    T_percent_scuba_trips = round(T_YEAR_TRIPS_SCUBA / T_YEAR_TRIPS, 4),
...
by_year_log_percentage <- by_year_log |>
  ...
  mutate(
    L_percent_scuba_trips = round(L_YEAR_TRIPS_SCUBA / L_YEAR_TRIPS, 4),
    TRIP_YEAR = as.numeric(TRIP_YEAR)
  )
...
log_all <- by_year_log_percentage |>
  ...
  mutate(L_PERCENT_SCUBA = round(L_TOTAL_TRIPS_SCUBA / L_TOTAL_TRIPS, 4))
...
combo_years <- by_year_log_percentage |>
  ...
  mutate(
    TIP_PERCENT_REPRESENTATION_ALL = round(T_YEAR_TRIPS / L_YEAR_TRIPS * 100, 2),
...
across_years_TvL <- log_all |>
  cross_join(tip_all) |>
  mutate(
    TIP_PERCENT_REPRESENTATION_ALL = round(T_TOTAL_TRIPS / L_TOTAL_TRIPS * 100, 2),
