...
# gant_data$LAND_STANDARD_GEAR_NAME <- as.factor(gant_data$LAND_STANDARD_GEAR_NAME, 
#                                             levels = gant_data$LAND_STANDARD_GEAR_NAME[order(gant_data$n)])
...
agr_den_NOgears <- 
  ggplot() +
  geom_density(aes(FL_CM, color = "length_data_1983_2022"),linewidth = 0.75, alpha = .2, data = length_data_1983_2022) +
...
length_data_gears <- length_data_final %>% 
  ...
  # filter(n() >= 30) %>% 
  ungroup() |> 
  filter(LAND_STANDARD_GEAR_NAME %in% c("POTS AND TRAPS; CMB", "POTS AND TRAPS; FISH",
...
length_data_gears_2012 <- length_data_glm_2012 %>% 
  ...
  # filter(n() >= 30) %>% 
  ungroup() |> 
  filter(LAND_STANDARD_GEAR_NAME %in% c("BY HAND; DIVING GEAR", 
                                         "SPEARS; DIVING")) 
...
agr_den_v12 <- length_data_gears_2012 %>% 
  ...
  theme(legend.title = element_text(size=14), 
        ...
        axis.title.y = element_text( size = 14))+
  guides(color=guide_legend(ncol = 2))+
  geom_vline(data=muv12, aes(xintercept=grp.mean, color=LAND_STANDARD_GEAR_NAME),
             linetype="dashed")
