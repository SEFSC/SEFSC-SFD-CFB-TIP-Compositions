...
TIP_gears <- read_csv("~/SEFSC-SFD-CFB-TIP-Compositions/data/CSVs/tip_gears_yts_pr_landstdgearname.csv",
                   show_col_types = FALSE)
...
min(pr_yt$LENGTH1_MM,na.rm = TRUE)
...
#### Extract marfin samples, look at island distribution - NO USVI MARFIN REEFFISH SAMPLING
...
```{r mgt_period_plot, fig.width=12, fig.height=12}
#| warning: false
#| column: page 
#| echo: false
...
```{r mgt_period_hl_plot, fig.width=12, fig.height=12}
#| warning: false
#| column: page 
#| echo: false
...
```{r mgt_period_trap_plot, fig.width=12, fig.height=12}
#| warning: false
#| column: page
#| echo: false
...
```{r mgt_period_net_plot, fig.width=12, fig.height=12}
#| warning: false
#| column: page
#| echo: false
...
length_data_final_cleaned <- length_data_final |> 
  filter(LAND_STANDARD_GEAR_NAME != "UNSPECIFIED GEAR")

ot<- length_data_final_cleaned[length_data_final_cleaned$gear_short=='OT',]
counts = ot %>%
  tabyl(LAND_STANDARD_GEAR_NAME) %>%
  mutate(n_labels = paste0(LAND_STANDARD_GEAR_NAME, " (n= ", n, ")" ))
...
#| echo: false
#| fig-cap: Stoplight Parrotfish LAND_STANDARD_GEAR_NAME PR
#| warning: false 
#| column: page 

# create count of observed records by individual gear name

pr_count <- length_data_final %>%
   add_count(COUNTY) %>%
...
# plot    
  ...
  ggplot(data = pr_gear_time, aes(x = INTERVIEW_DATE, y = LAND_STANDARD_GEAR_NAMEn , group = COUNTY ,color = COUNTYn )) + 
    facet_wrap(vars(COUNTYn), ncol = 4) +
    geom_point(size = 2) + 
    labs(x = "Year", y = "LAND_STANDARD_GEAR_NAME (# obs)", 
...
# create count of observed records for each GEAR GROUP
    pr_geargroup_time <- subset(pr_gear_time, select = -n) %>%
      add_count(gear) %>%
      mutate(gearn = paste0(gear, '(', n, ')')) 

# plot    

  ggplot(data = pr_geargroup_time, aes(x = INTERVIEW_DATE, y = gearn , group = COUNTY ,color = COUNTYn )) + 
    facet_wrap(vars(COUNTYn), ncol = 4) +
    geom_point(size = 2) + 
    labs(x = "Year", y = "LAND_STANDARD_GEAR_NAME (# obs)", 
         title = "Area-time distribution of landed gear PR", 
         color = "COUNTY (# obs)",
             subtitle = paste("N = ", nrow(pr_geargroup_time))) +
    theme(text = element_text(size=15),
          legend.position = "none")
