...
table(sttj_yt$LENGTH_TYPE1, useNA='always')
...
# Trip ID vs gear by year investigation 
## how many unique trip ID's per year
### group by year, count unique trip id's
When we remove data from years with less than 30 unique trip ID's, we are left with only 12 years of data. IMO too few for assessment
...
length_data_tripid <- length_data_final |> 
  group_by( YEAR, ID) #|> 
  # filter(n() >= 30)  |>  
  # ungroup () 
...
length_data_trip30 <- length_data_final |> 
  group_by(YEAR) |>
  filter(n_distinct(ID) >= 30)|>
  summarise(n_distinct(ID)) 
print.data.frame(length_data_trip30)

```

## how many records per gear per year
### group by year, count occurances of each gear
...
length_data_gearid <- length_data_final |> 
...
length_data_gearg29 <- length_data_final |> 
  group_by(YEAR, LAND_STANDARD_GEAR_NAME) |>
  filter(n() < 30)|>
  # ungroup() |> 
  # group_by(YEAR, gear) |> 
...
# plot    
  ...
  ggplot(data = sttj_gear_time, aes(x = INTERVIEW_DATE, y = LAND_STANDARD_GEAR_NAMEn , group = COUNTY ,color = COUNTYn )) + 
