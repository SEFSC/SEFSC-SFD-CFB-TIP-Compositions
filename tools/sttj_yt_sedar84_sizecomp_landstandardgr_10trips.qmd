...
Filtered to years with 10 or more unique trip IDs (regardless of gear) per year. STT and STJ records are grouped together.

Filter results in 1,831 less records than the 30 fish per year filter.
...
ycounts =length_data_final %>% group_by(ID) %>% filter(n() >= 10) %>% ungroup %>%
  tabyl(gear) %>%
  mutate(n_labels = paste0(gear, " (n= ", n, ")" ))

 length_data_final %>% group_by(ID) %>% filter(n() >= 10) %>% ungroup %>%
ggplot(aes(FL_CM))+
...
length_data_final_10 <- length_data_final %>%  
  group_by(ID) %>% 
  filter(n() >= 10) 
```
...
hl<- length_data_final_10[length_data_final$gear_short=='HL',]
counts = hl %>% 
